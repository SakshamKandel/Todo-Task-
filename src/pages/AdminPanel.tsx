...
export default function AdminPanel({ onClose }: AdminPanelProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-[9998] flex items-center justify-center backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col shadow-2xl">

                {/* Header */}
                <div className="px-8 py-6 border-b border-zinc-100 flex items-center justify-between bg-white shrink-0">
                    <div>
                        <p className="text-[11px] font-bold uppercase tracking-widest text-orange-500 mb-1">Management</p>
                        <h2 className="text-3xl font-bold text-zinc-900 uppercase tracking-tight">Admin Panel</h2>
                    </div>
                    <button onClick={onClose} className="w-12 h-12 flex items-center justify-center bg-zinc-100 text-zinc-500 hover:text-red-600 hover:bg-red-50 transition-colors">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>

                {/* Tabs */}
                <div className="px-8 py-4 border-b border-zinc-100 bg-zinc-50 flex items-center gap-2 overflow-x-auto shrink-0">
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id as any)}
                            className={`px-5 py-2 text-sm font-bold uppercase tracking-wide transition-all flex items-center gap-2 ${activeTab === tab.id ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-600 hover:bg-zinc-100'
                                }`}
                        >
                            {tab.label}
                            {tab.count !== null && (
                                <span className={`px-2 py-0.5 text-xs ${activeTab === tab.id ? 'bg-orange-500' : 'bg-zinc-200'}`}>
                                    {tab.count}
                                </span>
                            )}
                        </button>
                            ...
                            {activeTab === 'users' && (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between mb-6">
                                        <div>
                                            <h3 className="text-xl font-bold text-zinc-900 uppercase">All Users</h3>
                                            <p className="text-sm text-zinc-500">{users.length} registered users</p>
                                        </div>
                                        {canCreateUser('normal') && (
                                            <button onClick={() => setShowCreateUser(true)} className="px-6 py-3 bg-zinc-900 text-white font-bold text-sm uppercase tracking-wide hover:bg-orange-500 transition-colors flex items-center gap-2">
                                    ...
                                    {users.map(user => (
                                        <div key={user.id} className="flex items-center gap-4 p-5 bg-white border-l-4 border-l-zinc-200 hover:border-l-orange-500 transition-colors">
                                            <div className="w-12 h-12 bg-gradient-to-br from-zinc-200 to-zinc-300 flex items-center justify-center text-zinc-600 font-black text-xl uppercase">
                                            ...
                                            <span className={`px-3 py-1 text-[10px] font-black uppercase ${user.role === 'superadmin' ? 'bg-purple-100 text-purple-600' :
                                            ...
                                            {user.id !== profile?.id && isRole('superadmin') && (
                                                <div className="flex items-center gap-2">
                                                    <button
                                                        ...
                                                        className="p-2 bg-zinc-100 text-zinc-500 hover:bg-orange-100 hover:text-orange-600 transition-colors"
